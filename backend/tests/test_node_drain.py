...
def test_drain_does_not_release_a_node_whose_other_copies_include_another_draining_node(
    ...
    with ExitStack() as stack:
        ...
        fake_d.put(f"/chunks/{chunk_hash}", content=data)
        fake_e.put(f"/chunks/{chunk_hash}", content=data)
        ...
        assert fake_a.get(f"/chunks/{chunk_hash}").status_code == 200  # not released
        assert fake_spare1.get(f"/chunks/{chunk_hash}").status_code == 200  # repaired onto the spare
...
def test_draining_node_reports_zero_remaining_once_a_spare_takes_over(client, tmp_path, monkeypatch):
    ...
    with ExitStack() as stack:
        ...
        assert fake_a.get(f"/chunks/{chunk_hash}").status_code == 404  # released from a
        # Repair needs 2 more permanent replicas (RF 3, minus b) -- both spares get it.
        assert fake_spare1.get(f"/chunks/{chunk_hash}").status_code == 200
        assert fake_spare2.get(f"/chunks/{chunk_hash}").status_code == 200
...
def test_draining_node_reports_remaining_when_no_spare_exists(client, tmp_path, monkeypatch):
    ...
    with ExitStack() as stack:
        ...
        assert response.status_code == 200
        assert response.json() == {"remaining_chunks": 1}
        assert fake_a.get(f"/chunks/{chunk_hash}").status_code == 200  # not released -- still needed
